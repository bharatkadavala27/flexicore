    ...
    <AdminShell>
      <div className="space-y-5 max-w-4xl">
        <div className="flex items-center gap-3">
          <Link href="/admin/products" className="p-2 rounded-xl transition-colors hover:bg-white" style={{ color: '#555' }}>
            <ArrowLeft size={18} />
          </Link>
          <h1 className="text-2xl font-bold" style={{ color: '#1a1a1a' }}>Edit Product</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{ border: '1px solid rgba(0,0,0,0.06)' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className={inputCls} style={inputStyle} value={form.sku} onChange={e => setForm({ ...form, sku: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                ...
                <input className={inputCls} style={inputStyle} value={form.priceRange} onChange={e => setForm({ ...form, priceRange: e.target.value })} />
              </div>
              <div>
                <label className={labelCls} style={labelStyle}>Surface Type</label>
                <input className={inputCls} style={inputStyle} value={form.surface} onChange={e => setForm({ ...form, surface: e.target.value })} />
              </div>
              <div>
                <label className={labelCls} style={labelStyle}>Finish</label>
                <input className={inputCls} style={inputStyle} value={form.finish} onChange={e => setForm({ ...form, finish: e.target.value })} />
              </div>
              <div>
                <label className={labelCls} style={labelStyle}>Thickness</label>
                <input className={inputCls} style={inputStyle} value={form.thickness} onChange={e => setForm({ ...form, thickness: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                ...
                <input className={inputCls} style={inputStyle} value={form.videoUrl} onChange={e => setForm({ ...form, videoUrl: e.target.value })} />
              </div>
              <div className="flex items-center gap-3 mt-2">
                <input type="checkbox" id="visible" checked={form.isVisible} onChange={e => setForm({ ...form, isVisible: e.target.checked })} className="w-4 h-4 rounded" />
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{ border: '1px solid rgba(0,0,0,0.06)' }}>
            ...
            {previewUrls.length > 0 && (
              <div className="flex flex-wrap gap-3 mb-4">
                {previewUrls.map((url, i) => (
                  <div key={i} className="relative w-24 h-24 rounded-xl overflow-hidden" style={{ border: '2px solid #555' }}>
            ...
            <input type="file" accept="image/*" multiple className="text-sm" onChange={e => setImageFiles(Array.from(e.target.files || []))} />
          </div>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={saving}
              ...
              {saving ? <><Loader2 size={15} className="animate-spin" /> Saving...</> : 'Update Product'}
            </button>
